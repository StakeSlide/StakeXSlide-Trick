...
const blockHash = '00000000000000000000644330e1340fc6e894a95c37060bdd180ed11d068944';
...
updateButton.addEventListener('click', () => OnInputChange(true));
...
function GetChain(seed, amount = 1000, goodValue = 2) {
	...
	const seedToPoint = (seed) => {
		...
		const f = parseFloat((4294967296 / (dec + 1)) * (1 - 0.02));
...
function UpdateStatistics(totalCount = 0, goodCount = 0) {
	...
	statisticsDiv.innerHTML = `
		<span class="good">${goodCount}</span>/${totalCount} Wins 
		(<span class="good">${goodPercentage}%</span> Chance) • 
		<span class="bad">${lossCount}</span>/${totalCount} Losses 
		(<span class="bad">${lossPercentage}%</span> Chance)
	`;
}
